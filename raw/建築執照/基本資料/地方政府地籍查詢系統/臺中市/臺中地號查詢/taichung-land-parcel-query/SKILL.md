...
- The cultural heritage query also does a best-effort color scan of its screenshot for a nearby designated monument (古蹟) fill color; it only reliably detects that one category (others blend too closely with the basemap to detect automatically), so the report also always includes a reminder to visually check the screenshot against the map legend for other categories.
