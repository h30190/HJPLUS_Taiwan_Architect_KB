# 設計軟體與工具 (Design Software & Tools)

## Skills

* [BIM工具 — Archicad 工作流程](BIM工具/Archicad/archicad-basic-workflow/SKILL.md) - Understanding Archicad project setup, BIM modeling workflow, view organization, drawing production, and publishing processes
* [BIM工具 — Archicad 範本設定](BIM工具/Archicad/archicad-template-setup/SKILL.md) - Setting up Archicad templates for Taiwan architectural practice
* [BIM工具 — Archicad 文件產出](BIM工具/Archicad/archicad-documentation/SKILL.md) - Archicad documentation and drawing production workflows
* [BIM工具 — Archicad IFC 協調](BIM工具/Archicad/archicad-ifc-coordination/SKILL.md) - IFC-based coordination workflows in Archicad
* [BIM工具 — Revit AI 目標判定與安全執行](BIM工具/Revit/AI目標判定與安全執行/revit-ai-targeting-and-safe-execution/SKILL.md) - Revit AI 任務的目標證明、分級唯讀檢查、安全執行、獨立讀回與證據回報

## Planned

* [CAD繪圖工具](CAD繪圖工具/) - AutoCAD 等 CAD 工具（籌備中）
* [渲染與即時視覺化工具](渲染與即時視覺化工具/) - Enscape, Lumion, Twinmotion 等（籌備中）
* [三維建模與視覺化工具](三維建模與視覺化工具/) - SketchUp, Blender, 3ds Max 等（籌備中）
* [BIM協調與雲端協作工具](BIM協調與雲端協作工具/) - BIM 360, Navisworks, Solibri 等（籌備中）
* [平面設計與排版工具](平面設計與排版工具/) - Photoshop, Illustrator, InDesign 等（籌備中）
* [即時互動與遊戲引擎工具](即時互動與遊戲引擎工具/) - Unreal Engine, Unity 等（籌備中）
* [辦公文件與簡報工具](辦公文件與簡報工具/) - Word, Excel, PowerPoint 等（籌備中）
* [影音後期與動態影像工具](影音後期與動態影像工具/) - Premiere Pro, After Effects 等（籌備中）
* [AI生成與節點工具](AI生成與節點工具/) - ComfyUI 等 AI 工具（籌備中）
* [協同審查與PDF工具](協同審查與PDF工具/) - 協同審查與 PDF 工具（籌備中）
* [參數化與建模工具](參數化與建模工具/) - 參數化設計工具（籌備中）
* [基地掃描與實景建模工具](基地掃描與實景建模工具/) - 基地掃描與實景建模（籌備中）
* [GIS與基地分析工具](GIS與基地分析工具/) - GIS 與基地分析工具（籌備中）
* [團隊協作與知識管理工具](團隊協作與知識管理工具/) - 團隊協作與知識管理（籌備中）
