...
- [ ] **不要修改 README.md**：技能計數與「最近更新」由維護者與 CI 維護，詳見下節。
...
```bash
python scripts/validate_okf.py     # OKF schema
python scripts/check_content.py    # 相對連結、去識別化、README 計數同步
```

CI 在 PR 上跑的是同一支腳本，本機過了 CI 就會過。若 `check_content.py` 誤報（例如把政府函號當成電話號碼、把公開案例中的事務所名當成個資），請在 PR 說明指出，維護者會調整規則——**不要為了讓檢查通過而刪掉正確的內容**。

### 請不要修改 README.md

README 的**技能分類計數表**與**「最近更新」**兩個區塊，由維護者在合併後統一處理（計數另有 CI 自動重算）。

這不是格式潔癖，而是因為**那是整個庫最容易衝突的地方**：你送出 PR 之後，只要 main 又合併了其他人的 PR，計數就會對不上，你就得再 rebase 一次。你只要專注在自己的技能檔案，就算落後幾十個 commit 也不會衝突。

如果你已經在 fork 上改了，把那部分的改動撤掉即可，不影響你其他內容的審查。
...
`python scripts/check_content.py` 會掃描下列樣式並在 PR 階段擋下：身分證字號、統一編號、行動電話、email、建照號碼、地號、與專案關聯的事務所名、承辦人姓名、本機絕對路徑（`C:\Users\…`、`/Users/…`）、以及 API 金鑰或密碼樣式。

但腳本只能抓固定樣式，**真正的判斷還是在你**。實務上最常見的三種夾帶：

1. **檔案路徑洩漏專案名**：`C:\Users\王大明\Desktop\OO建設信義案\圖說.dwg` —— 路徑本身就包含業主、案名與你的姓名。
2. **截圖沒有遮蔽**：地籍圖、建照存根、系統畫面上的地號與案號。
3. **範例沿用真實案件**：worked example 直接拿手上的案子改寫，尺寸與地號都還是原案的。

#### 內部文件當來源時，誠實標示它

事務所內部的實務經驗（送件流程、表單代碼、對副本清單）往往是最有價值的內容，但它們**沒有公開出處可查證**。這種情況不要硬掰一個官方來源，而是誠實標示：

```yaml
sources:
  - id: firm-sop-taipei-permit
    resource: urn:firm-internal:taipei-permit-sop
    title: 事務所內部臺北市建照送件實務文件（圖冊開號慣例、無紙化平台檔名前綴、
           協審對副本攜帶清單）—— 專案識別資訊與承辦人姓名已去識別化
    last_modified: 2026-05-05
```

`urn:firm-internal:` 這個寫法明確表達「這不是公開可查證的來源」，讀者就知道要自行向主管機關再確認一次。內文引用時用腳註指回這個來源，並在該段標明它是**行政慣例而非成文規定**。

這個做法來自 [#69](https://github.com/h30190/HJPLUS_Taiwan_Architect_KB/pull/69)，可作為範例參考。
